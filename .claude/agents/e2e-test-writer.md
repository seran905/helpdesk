...
- **`playwright.config.ts`** defines two `webServer` entries — `server/` (port 3001) and `client/` (port 5173) — rather than expecting them to already be running. The server entry's `env` is the same parsed `server/.env.test` values, merged on top of the inherited process env (Playwright merges as `{...process.env, ...webServer.env}`), so that specific spawned process talks to `helpdesk_test` regardless of what `server/.env` says.
- **Port conflict gotcha**: the client's API base URL is hardcoded to `http://localhost:3001` (no Vite dev proxy, no env-driven override — see `client/src/lib/auth-client.ts`), so E2E must run on the same ports (3001/5173) as manual dev servers. Both `webServer` entries use `reuseExistingServer: false` on purpose — E2E never silently attaches to an already-running dev-DB-backed server, it fails to bind instead. Practically: **stop any manually-running `npm run dev` in `client/`/`server/` before running `npm run test:e2e`** from the repo root.
